...
<Steps>
    ...
    **Optional.** Click to **Enable external ticket processing**.
    ...
    <Step>
    **Optional.** In the **Project keys** field, enter a comma-separated list of Jira Cloud project keys to limit which projects are used when creating external tickets. If left empty, all projects accessible to the service account will be used.

    <Tip>
    Scoping to specific project keys avoids `403` errors when the service account can't view statuses or browse every project in your Jira instance, and lets you grant narrower permissions.
    </Tip>

    <Warning>
    **Listing more than 50 project keys causes external ticket processing to fail.** Jira Cloud accepts at most 50 project keys when filtering projects, so keep the list at 50 keys or fewer.
    </Warning>
    </Step>
    <Step>
    Click **Save**.
