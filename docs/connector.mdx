...
Both mappings are required. If the `products` mapping is missing, the Atlassian API rejects the request with an HTTP 400 error, and the connector reports this as a generic "create account failed" error without surfacing the upstream message, so the cause may not be immediately obvious.
