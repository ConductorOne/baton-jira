...
func (j *Jira) getJiraStatusesForProject(ctx context.Context, projectId string) ([]jira.JiraStatus, error) {
	...
	for {
		...
		if err != nil {
			...
			return nil, wrapError(err, fmt.Sprintf("error getting statuses for project %s", projectId), statusCode)
		}
...
func (j *Jira) ListTicketSchemas(ctx context.Context, p *pagination.Token) ([]*v2.TicketSchema, string, annotations.Annotations, error) {
	...
	for _, project := range filteredProjects {
		...
		if err != nil {
			return nil, "", nil, wrapError(err, fmt.Sprintf("failed to get ticket statuses for project %s (ID: %s)", project.Key, project.ID), nil)
		}
